...
export default function SegmentError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  ...
  return (
    <main className="min-h-[70vh] bg-brand-bgLight px-4 py-20">
      <div className="mx-auto max-w-xl rounded-none border border-brand-secondary/30 bg-cream-50 p-8 text-center shadow-sm">
        <div className="mx-auto mb-5 flex h-16 w-16 items-center justify-center rounded-none bg-brand-primary/10 text-brand-primary">
        ...
        <div className="mt-7 flex flex-col justify-center gap-3 sm:flex-row">
          <button onClick={reset} className="rounded-none bg-brand-primary px-7 py-3 font-semibold text-white hover:bg-brand-bgDark">Try again</button>
          <Link href="/" className="rounded-none border border-brand-primary px-7 py-3 font-semibold text-brand-primary hover:bg-brand-bgLight">Back home</Link>
        </div>
      </div>
    </main>
  );
