          ...
          <div className="grid gap-10 lg:grid-cols-[1.05fr_0.95fr]">
            ...
            <div className="lg:sticky lg:top-24 lg:self-start">
              <div className="rounded-[2rem] border border-brand-border bg-white/86 p-6 shadow-[0_24px_80px_-62px_rgba(36,31,34,0.45)] backdrop-blur md:p-8">
                <p className="font-label text-xs font-bold uppercase tracking-[0.22em] text-brand-primary">{product.brand}</p>
                <h1 className="mt-2 font-display text-4xl font-semibold leading-tight text-brand-textPrimary md:text-5xl">{product.name}</h1>
                ...
                <div className="mt-5 flex flex-wrap gap-2">{product.madeInNepal && <span className="font-label rounded-full bg-brand-primary px-3 py-1 text-xs font-bold uppercase tracking-[0.14em] text-white">Made in Nepal</span>}{product.concernTags.map((tag) => <span key={tag} className="rounded-full bg-brand-primary-light px-3 py-1 text-xs font-bold text-brand-primary">{tag}</span>)}</div>
                <div className="mt-6 flex items-end gap-3"><span className="font-display text-4xl font-semibold text-brand-gold">{formatNPR(product.price)}</span>{product.originalPrice && <span className="pb-1 text-brand-textMuted line-through">{formatNPR(product.originalPrice)}</span>}</div>
                <div className="mt-5 grid grid-cols-2 gap-3 text-sm text-brand-textMuted md:grid-cols-4"><InfoPill label="Size" value={product.size} /><InfoPill label="Origin" value={product.origin} /><InfoPill label="Stock" value={product.inStock ? (isLowStock ? `Only ${product.stockCount} left` : "In stock") : "Out of stock"} /><button onClick={copySku} aria-label={`Copy SKU ${product.sku}`} className="rounded-2xl border border-brand-border bg-brand-bgLight p-3 text-left font-bold text-brand-primary"><Copy size={14} className="mb-1" />{product.sku}</button></div>
...
function InfoPill({ label, value }: { label: string; value: string }) { return <div className="rounded-2xl border border-brand-border bg-brand-bgLight p-3"><p className="font-label text-[10px] font-bold uppercase tracking-[0.16em] text-brand-primary">{label}</p><p className="mt-1 font-bold text-brand-textPrimary">{value}</p></div>; }
...
function Story({ title, body }: { title: string; body: string }) { return <div className="rounded-[2rem] border border-brand-border bg-white p-6 shadow-sm"><h2 className="font-display text-3xl font-semibold text-brand-textPrimary">{title}</h2><p className="mt-3 text-sm leading-7 text-brand-textMuted">{body}</p></div>; }
