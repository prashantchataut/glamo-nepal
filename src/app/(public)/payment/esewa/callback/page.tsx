...
export default function EsewaCallbackPage() {
  ...
  useEffect(() => {
    ...
    const token = refId || transactionUuid;

    verifyPayment(transactionUuid, "esewa", token)
      .then((result) => {
        if (result.data) {
          setStatus("success");
          setTimeout(() => router.push(`/order-confirmation/${transactionUuid}`), 1500);
        } else {
          setStatus("failed");
        }
      })
      ...
      <div className="text-center">
        {status === "verifying" && (
          <>
            ...
            <p className="mt-4 text-sm text-brand-textMuted">Verifying your eSewa payment...</p>
          </>
        )}
        {status === "success" && (
          <>
