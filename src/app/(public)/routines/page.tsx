        ...
        <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
          {bundles.map((bundle) => (
            <Link key={bundle.slug} href={`/routines/${bundle.slug}`} className="group overflow-hidden rounded-[2rem] border border-neutral-200 bg-white shadow-sm transition duration-300 hover:-translate-y-1 hover:border-primary/30 hover:shadow-card-hover focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 outline-none">
              ...
              <div className="p-6">
                <p className="font-label text-xs font-bold uppercase tracking-[0.2em] text-primary">{bundle.eyebrow}</p>
                <h2 className="mt-2 font-display text-2xl font-semibold text-neutral-900 group-hover:text-primary transition-colors">{bundle.title}</h2>
                <p className="mt-2 text-sm leading-6 text-neutral-500 line-clamp-2">{bundle.description}</p>
                <div className="mt-3 flex flex-wrap gap-1.5">
                  {bundle.skinTypes.slice(0, 3).map((tag) => (
                    <span key={tag} className="font-label rounded-full bg-primary/10 px-2 py-0.5 text-[10px] font-bold uppercase tracking-widest text-primary">{tag}</span>
                  ))}
                </div>
                <p className="mt-3 text-xs font-semibold text-primary">{bundle.products.length} products &middot; Save {formatNPR(bundle.savings)}</p>
              </div>
            </Link>
          ))}
        </div>
