    ...
    <main className="bg-cream-50">
      <section className="border-b border-cream-200 bg-cream-50">
        <div className="mx-auto grid max-w-7xl gap-10 px-4 py-12 md:px-6 md:py-16 lg:grid-cols-[0.85fr_1.15fr] lg:items-center lg:px-8">
          <div className="relative aspect-[4/3] overflow-hidden border border-cream-200 bg-cream-50">
          ...
          <div>
            <Link href="/brands" className="text-sm font-semibold text-brand-rose">← All brands</Link>
            <p className="type-label mt-6 text-brand-rose">Brand edit</p>
            <h1 className="mt-4 font-display text-5xl font-medium leading-[0.95] text-ink md:text-7xl">{brand.name}</h1>
            <p className="mt-6 max-w-2xl text-base leading-8 text-cream-700">{brand.description}</p>
            <div className="mt-8 grid gap-3 sm:grid-cols-3">
              <div className="border border-cream-200 bg-cream-50 p-4"><p className="text-2xl font-semibold text-ink">{brand.productCount}</p><p className="text-xs uppercase tracking-[0.14em] text-cream-400">Products</p></div>
              <div className="border border-cream-200 bg-cream-50 p-4"><p className="text-2xl font-semibold text-ink">{brand.madeInNepalCount}</p><p className="text-xs uppercase tracking-[0.14em] text-cream-400">Made in Nepal</p></div>
              <div className="border border-cream-200 bg-cream-50 p-4"><p className="text-2xl font-semibold text-ink">{brand.categories.length}</p><p className="text-xs uppercase tracking-[0.14em] text-cream-400">Categories</p></div>
            </div>
          </div>
        </div>
      </section>
      <EditorialSection title={`Shop ${brand.name}`} description="Product cards use GLAMO's standard storefront layout for consistent browsing, price comparison and quick add behavior.">
        {products.length ? <div className="grid grid-cols-2 gap-4 md:gap-6 lg:grid-cols-4">{products.map((product) => <ProductCard key={product.id} product={product} />)}</div> : <div className="border border-cream-200 bg-cream-50 p-10 text-center"><h2 className="font-display text-3xl text-ink">This brand edit is coming soon.</h2><p className="mt-3 text-cream-700">Check back for new arrivals or browse the full shop.</p><Link href="/shop" className="mt-6 inline-flex bg-brand-rose px-7 py-3 text-xs font-semibold uppercase tracking-[0.16em] text-white">Browse shop</Link></div>}
      </EditorialSection>
    </main>
  );
}
