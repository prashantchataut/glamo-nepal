...
export default function AdminLoginPage() {
  return (
    <div className="min-h-screen bg-cream-50 px-4 py-10 md:px-8">
      <div className="mx-auto grid min-h-[calc(100vh-5rem)] max-w-5xl items-center gap-0 md:grid-cols-[0.9fr_1.1fr]">
        <section className="relative overflow-hidden bg-ink p-8 text-white md:p-12">
          <div className="pointer-events-none absolute inset-0 bg-[linear-gradient(135deg,rgba(255,255,255,0.08),transparent_55%)]" />
          <div className="relative z-10">
            <span className="type-label inline-flex items-center gap-2 border border-white/15 bg-cream-50/10 px-4 py-2 text-xs font-bold uppercase tracking-[0.18em] text-white/80">
        ...
        <section className="border border-cream-200 bg-cream-50 p-6 md:p-10">
          <Suspense fallback={
            <div className="flex flex-col items-center justify-center py-16" aria-label="Loading admin login form">
              <div className="h-10 w-10 animate-spin rounded-none border-4 border-brand-rose border-t-transparent" />
              <p className="mt-4 text-sm text-cream-400">Loading admin login...</p>
            </div>
          }>
            <AdminLoginForm />
          </Suspense>
