...
export default function ProductDetailClient({ product, related }: { product: Product; related: Product[] }) {
  ...
  return (
    <div className="bg-brand-bgLight">
          ...
          <div className="grid gap-10 lg:grid-cols-[1.05fr_0.95fr]">
            ...
            <div className="lg:sticky lg:top-24 lg:self-start">
              <div className="rounded-[2rem] border border-brand-border bg-white/86 p-6 shadow-[0_24px_80px_-62px_rgba(36,31,34,0.45)] backdrop-blur md:p-8">
                ...
                <div ref={ctaRef} className="mt-7 flex flex-col gap-3 sm:flex-row"><button onClick={addToCart} disabled={!product.inStock} className="flex flex-1 items-center justify-center gap-2 rounded-full bg-brand-primary px-7 py-4 font-bold text-white shadow-lg shadow-brand-primary/20 transition hover:bg-brand-primary-hover disabled:cursor-not-allowed disabled:bg-brand-textMuted"><ShoppingBag size={18} /> Add to cart</button><button onClick={share} className="flex items-center justify-center gap-2 rounded-full border border-brand-primary px-7 py-4 font-bold text-brand-primary hover:bg-brand-primary-light"><Share2 size={18} /> Share</button></div>
                <Dialog><DialogTrigger asChild><button className="mt-3 text-sm font-bold text-brand-primary underline underline-offset-4">Open size guide</button></DialogTrigger><DialogContent><DialogHeader><DialogTitle className="font-serif text-2xl">GLAMO size guide</DialogTitle></DialogHeader><div className="space-y-3 text-sm text-brand-textMuted"><p>Use this guide for product-specific size, shade and usage guidance.</p><p>Skincare sizes are listed in ml/g. Makeup shade availability is shown per variant when data is connected.</p></div></DialogContent></Dialog>
              </div>
      ...
      <div className={cn("fixed inset-x-0 bottom-16 z-50 border-t border-brand-border bg-white/95 p-3 shadow-2xl backdrop-blur transition-transform md:hidden", showSticky ? "translate-y-0" : "translate-y-full")}><div className="flex items-center gap-3"><div className="min-w-0 flex-1"><p className="line-clamp-1 font-bold text-brand-textPrimary">{product.name}</p><p className="text-brand-gold">{formatNpr(product.price)}</p></div><button onClick={addToCart} className="rounded-full bg-brand-primary px-5 py-3 font-bold text-white">Add</button></div></div>
    </div>
  );
}
