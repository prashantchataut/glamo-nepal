      ...
      <div className="container mx-auto px-4 py-8 md:px-6">
        ...
        <div className="grid gap-10 lg:grid-cols-2">
          ...
          <div className="lg:sticky lg:top-24 lg:self-start">
            ...
            <p className="mt-5 text-lg leading-relaxed text-brand-textMuted">{product.description}</p>
        ...
        <section className="mt-8 grid gap-4 rounded-[2rem] border border-amber-300/60 bg-amber-50 p-6 text-sm text-amber-900 md:grid-cols-3"><div><strong>Patch test:</strong><p className="mt-1 leading-6">{getPatchTestNote(product)}</p></div><div><strong>Batch/expiry:</strong><p className="mt-1 leading-6">{getBatchExpiryNote(product)}</p></div><div><strong>Final approval:</strong><p className="mt-1 leading-6">Descriptions, claims, images and MRP are mock until supplier-approved.</p></div></section>
        <section className="mt-10 rounded-[2rem] bg-white p-6"><h2 className="font-serif text-3xl font-semibold text-brand-textPrimary">Product FAQ</h2><Accordion type="single" collapsible className="mt-4"><AccordionItem value="delivery"><AccordionTrigger>Can I pay with Cash on Delivery?</AccordionTrigger><AccordionContent>COD availability depends on district and courier rules. The checkout page includes a reusable COD checker for mock availability.</AccordionContent></AccordionItem><AccordionItem value="photo"><AccordionTrigger>Can I upload a photo review?</AccordionTrigger><AccordionContent><div className="rounded-2xl border border-dashed border-brand-secondary/40 bg-brand-bgLight p-4 text-brand-textMuted"><Gift className="mb-2" /> Photo review upload is coming soon. Backend storage and moderation are required before production.</div></AccordionContent></AccordionItem><AccordionItem value="claims"><AccordionTrigger>Are product claims final?</AccordionTrigger><AccordionContent>No. Product data is frontend mock data and must be replaced with supplier-approved claims, ingredients and images.</AccordionContent></AccordionItem></Accordion></section>
        <section className="mt-14"><div className="mb-6"><p className="text-xs font-bold uppercase tracking-[0.22em] text-brand-gold">Routine pairings</p><h2 className="font-serif text-3xl font-semibold text-brand-textPrimary">Bundle-ready recommendations</h2></div><div className="grid gap-6 md:grid-cols-2">{recommendedBundles.map((bundle) => <ProductBundleCard key={bundle.slug} bundle={bundle} compact />)}</div></section>
        <section className="mt-14"><div className="mb-6"><p className="text-xs font-bold uppercase tracking-[0.22em] text-brand-gold">More to explore</p><h2 className="font-serif text-3xl font-semibold text-brand-textPrimary">Related products</h2></div><div className="grid grid-cols-2 gap-4 md:grid-cols-4 md:gap-6">{related.map((p) => <ProductCard key={p.id} product={p} />)}</div></section>
      </div>
      <RecentlyViewedStrip excludeSlug={product.slug} />
      <div className={cn("fixed inset-x-0 bottom-0 z-50 border-t border-brand-secondary/20 bg-white/95 p-3 shadow-2xl backdrop-blur transition-transform md:hidden", showSticky ? "translate-y-0" : "translate-y-full")}><div className="flex items-center gap-3"><div className="min-w-0 flex-1"><p className="line-clamp-1 font-semibold text-brand-textPrimary">{product.name}</p><p className="text-brand-gold">{formatNpr(product.price)}</p></div><button onClick={addToCart} className="rounded-full bg-brand-primary px-5 py-3 font-semibold text-white">Add</button></div></div>
    </div>
  );
}
