...
export function MobileBottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 border-t border-border/50 bg-white/95 backdrop-blur-md md:hidden safe-area-bottom" aria-label="Mobile navigation">
      <div className="flex h-16 items-center justify-around">
        {navItems.map((item) => {
          ...
          if (item.action === "search") {
            return (
              <button key={item.label} type="button" onClick={openSearchModal} className="relative flex min-h-[44px] min-w-[44px] flex-col items-center justify-center gap-0.5 text-brand-textMuted transition-colors hover:text-brand-primary" aria-label="Open search">
