...
export function ShopFilterSidebar({ filters, onFilterChange, priceRange }: Props) {
  ...
  return (
    <aside className="space-y-6 rounded-[2rem] border border-brand-secondary/20 bg-white p-5 shadow-sm">
      ...
      <label className="block space-y-2 text-sm font-semibold text-brand-textPrimary">
        Search
        <input value={filters.search} onChange={(event) => onFilterChange({ ...filters, search: event.target.value })} placeholder="Serum, SPF, lipstick..." className="w-full rounded-2xl border border-border bg-brand-bgLight px-4 py-3 text-sm font-normal outline-none focus:ring-2 focus:ring-brand-primary/30" />
      </label>
      ...
      <div>
        ...
        <div className="space-y-2">
          {CATEGORIES.map((cat) => (
            <button key={cat.slug} type="button" onClick={() => onFilterChange({ ...filters, category: filters.category === cat.slug ? "" : cat.slug, subCategory: "" })} className={cn("w-full rounded-2xl px-4 py-2 text-left text-sm transition", filters.category === cat.slug ? "bg-brand-primary text-white" : "bg-brand-bgLight text-brand-textMuted hover:text-brand-primary")}>
              {cat.name}
            </button>
          ))}
        </div>
        ...
        <div className="max-h-52 space-y-2 overflow-auto pr-1">
          {BRANDS.map((brand) => (
            <label key={brand} className="flex items-center gap-2 text-sm text-brand-textMuted">
              <input type="checkbox" checked={filters.brands.includes(brand)} onChange={() => onFilterChange({ ...filters, brands: toggle(filters.brands, brand) })} className="h-4 w-4 rounded border-border text-brand-primary" />
              {brand}
            </label>
          ))}
        </div>
        ...
        <div className="space-y-2">
          {SKIN_TYPES.map((skinType) => (
            <label key={skinType} className="flex items-center gap-2 text-sm text-brand-textMuted">
              <input type="checkbox" checked={filters.skinType.includes(skinType)} onChange={() => onFilterChange({ ...filters, skinType: toggle(filters.skinType, skinType) })} className="h-4 w-4 rounded border-border text-brand-primary" />
              {skinType}
            </label>
          ))}
        </div>
      ...
      <div className="space-y-3">
        <label className="flex items-center gap-2 text-sm font-semibold text-brand-textPrimary"><input type="checkbox" checked={filters.madeInNepal} onChange={(event) => onFilterChange({ ...filters, madeInNepal: event.target.checked })} /> Made in Nepal only</label>
        <label className="flex items-center gap-2 text-sm font-semibold text-brand-textPrimary"><input type="checkbox" checked={filters.inStock} onChange={(event) => onFilterChange({ ...filters, inStock: event.target.checked })} /> In stock only</label>
      </div>
