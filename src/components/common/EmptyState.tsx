...
export function EmptyState({ variant, className, query }: { variant: Variant; className?: string; query?: string }) {
  ...
  return <div className={cn("flex flex-col items-center justify-center rounded-3xl bg-white/75 px-6 py-16 text-center", className)}>
    ...
    <p className="mt-8 text-xs font-bold uppercase tracking-widest text-brand-primary">GLAMO Nepal</p>
    <h3 className="mt-2 font-serif text-3xl font-semibold leading-tight text-brand-textPrimary md:text-4xl">{variant === "search" && query ? `No results for “${query}”` : config.title}</h3>
