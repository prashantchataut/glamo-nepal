...
export function RouteError({ title = "This GLAMO page needs a refresh", description = "Something interrupted this section. Try again or continue shopping.", reset, error }: { title?: string; description?: string; reset?: () => void; error?: Error & { digest?: string } }) {
  return (
    <main className="min-h-[70vh] bg-brand-bgLight px-4 py-16">
      <div className="mx-auto max-w-3xl overflow-hidden rounded-2xl border border-brand-border bg-cream-50 text-center shadow-editorial">
        <div className="bg-[var(--gradient-editorial)] px-6 py-10 md:px-10 md:py-12">
          <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-2xl bg-cream-50 text-amber-700 shadow-sm ring-1 ring-brand-border"><AlertTriangle size={32} /></div>
          ...
          <div className="mt-8 flex flex-wrap justify-center gap-3">
            {reset ? <button onClick={reset} className="inline-flex items-center gap-2 rounded-2xl bg-brand-primary px-6 py-3 font-semibold text-white transition hover:bg-brand-primary-hover"><RefreshCcw size={17} /> Try again</button> : null}
            <Link href="/shop" className="inline-flex items-center gap-2 rounded-2xl border border-brand-primary/25 bg-cream-50 px-6 py-3 font-semibold text-brand-primary transition hover:bg-brand-primary-light"><ShoppingBag size={17} /> Back to shop</Link>
          </div>
        </div>
      </div>
    </main>
