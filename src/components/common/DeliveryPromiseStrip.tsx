    ...
    <section aria-label="GLAMO delivery and trust promises" className="border-y border-black/5 bg-cream-50">
      <div className="container mx-auto grid gap-3 px-4 py-5 md:grid-cols-2 md:px-6 xl:grid-cols-4">
        {promises.map((promise) => (
            <div key={promise.title} className="flex items-start gap-3 rounded-none bg-brand-surfacePink px-4 py-4 ring-1 ring-black/5 transition-shadow duration-300 hover:shadow-sm">
              <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-none bg-cream-50 text-brand-primary shadow-sm">
                {promise.icon}
              </div>
              <div>
