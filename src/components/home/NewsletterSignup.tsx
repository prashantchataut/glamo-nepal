  ...
  return (
    <section className="bg-neutral-950 section-padding" aria-labelledby="newsletter-heading">
      <div className="mx-auto grid max-w-7xl gap-8 px-4 sm:px-6 lg:grid-cols-[1fr_1fr] lg:items-center lg:gap-16 lg:px-8">
        <div>
          <h2 id="newsletter-heading" className="type-display-md font-medium tracking-[-0.02em] text-neutral-50 md:type-display-lg">New drops, better routines.</h2>
          <p className="mt-4 max-w-md text-[0.9375rem] leading-[1.7] text-neutral-50/80">Curated product edits, restock notes and Kathmandu Valley delivery updates. No spam, no noisy beauty myths.</p>
        </div>
        <div aria-live="polite" aria-atomic="true">
          {submitted ? (
            <div className="rounded-[1.5rem] border border-white/10 bg-white/5 p-8" role="status">
              <p className="font-display text-2xl text-neutral-50">You&apos;re on the list.</p>
              <p className="mt-2 text-sm leading-6 text-neutral-50/80">We&apos;ll send the next GLAMO edit to your inbox.</p>
            </div>
          ) : (
            <form onSubmit={handleSubmit} method="POST" action="/api/newsletter" noValidate className="flex flex-col gap-3 sm:flex-row sm:gap-3" aria-busy={submitting || undefined}>
              ...
              <input id="newsletter-email" type="email" value={email} onChange={(e) => { setEmail(e.target.value); setError(""); }} placeholder="Your email address" required aria-invalid={error ? "true" : undefined} aria-describedby={error ? "newsletter-email-error" : undefined} disabled={submitting} autoComplete="email" className="min-h-[52px] flex-1 rounded-full border border-white/15 bg-white/10 px-6 text-sm text-neutral-50 placeholder:text-neutral-50/55 focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20" />
              <button type="submit" disabled={submitting} aria-disabled={submitting || undefined} className="min-h-[52px] rounded-full bg-primary px-8 text-sm font-semibold text-neutral-50 transition-colors hover:bg-primary-dark disabled:opacity-40">{submitting ? "Joining..." : "Join"}</button>
            </form>
          )}
