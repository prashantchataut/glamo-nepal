      ...
      <div className="container mx-auto px-4 md:px-6">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 md:mb-12 gap-4 md:gap-6">
          ...
          <Link href="/blog" className="group flex items-center gap-2 text-brand-primary font-semibold hover:text-brand-bgDark transition-colors duration-300 shrink-0">
            Read All Articles
            <span className="p-2 bg-brand-primary/10 rounded-2xl group-hover:bg-brand-primary group-hover:text-white transition-all duration-300">
              <MoveRight size={16} />
            </span>
          </Link>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
          {BLOG_POSTS.map((post, i) => (
            <Link key={post.id} href={`/blog/${post.slug}`} className="group block bg-cream-50 rounded-2xl overflow-hidden shadow-sm hover:shadow-soft transition-all duration-500 border border-border/30 hover:-translate-y-1 cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-primary focus-visible:ring-offset-2" style={{ animationDelay: `${i * 100}ms` }}>
              <div className="relative aspect-[3/2] overflow-hidden">
                ...
                <div className="absolute top-4 left-4">
                  <span className="font-label bg-cream-50/90 text-brand-primary text-[10px] font-bold uppercase tracking-[0.15em] px-3 py-1.5 rounded-2xl shadow-sm">
                    {post.category}
                  </span>
                </div>
              </div>
