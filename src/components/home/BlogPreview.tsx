      ...
      <div className="container mx-auto px-4 md:px-6">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 md:mb-12 gap-4 md:gap-6">
          <div className="max-w-2xl">
            <h2 id="blog-preview-heading" className="font-display text-3xl md:text-4xl lg:text-5xl font-semibold mb-3 md:mb-4 text-neutral-900">
              Glow Tips & <span className="text-primary italic">Beauty Secrets</span>
            </h2>
          ...
          {BLOG_POSTS.map((post, i) => (
            <Link key={post.id} href={`/blog/${post.slug}`} className="group block bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-card transition-all duration-500 border border-neutral-200/30 hover:-translate-y-1 cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-primary focus-visible:ring-offset-2" style={{ animationDelay: `${i * 100}ms` }}>
              ...
              <div className="p-6 md:p-8">
                <h3 className="font-display text-xl md:text-2xl font-semibold text-neutral-900 mb-3 line-clamp-2 group-hover:text-primary transition-colors duration-300 leading-tight">
                  {post.title}
                </h3>
