...
function StatCard({ label, value, note, icon: Icon }: { label: string; value: string | number; note: string; icon: ComponentType<{ size?: number | string; className?: string }> }) {
  return (
    <div className="card-hover rounded-2xl border border-brand-border bg-cream-50 p-6 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
      <div className="flex items-center justify-between gap-4">
        <div className="rounded-2xl bg-brand-primary-light p-3 text-brand-primary">
          <Icon size={18} />
        </div>
        <span className="font-label rounded-2xl bg-brand-bgLight px-3 py-1 text-xs font-bold uppercase tracking-[0.14em] text-brand-textMuted">Live</span>
      </div>
...
function StatusPill({ children, className }: { children: ReactNode; className: string }) {
  return <span className={cn("inline-flex rounded-2xl px-3 py-1 text-xs font-semibold ring-1", className)}>{children}</span>;
}
...
function MiniBar({ label, value, max }: { label: string; value: number; max: number }) {
  ...
  return (
    <div>
      ...
      <div className="h-1.5 rounded-2xl bg-brand-bgLight">
        <div className="h-1.5 rounded-2xl bg-brand-primary" style={{ width: `${width}%` }} />
      </div>
    </div>
  );
}
    ...
    <div className="overflow-hidden rounded-2xl border border-white/20 bg-brand-bgDark text-white shadow-lg">
      <div className="grid min-h-[180px] md:grid-cols-[1.1fr_0.9fr]">
        <div className="flex flex-col justify-center p-5 md:p-6">
          <span className="font-label w-fit rounded-2xl bg-cream-50/12 px-3 py-1 text-xs font-bold uppercase tracking-[0.18em] text-white/75">{banner.status}</span>
          ...
          <Link href={banner.href} className="mt-4 w-fit rounded-2xl bg-cream-50 px-4 py-2 text-sm font-bold text-brand-primary">
            {banner.cta}
          </Link>
        </div>
        ...
        <div className="flex h-full flex-col">
          <div className="flex items-center justify-between border-b border-brand-border px-6 py-6">
            <Link href="/admin" className="flex items-center gap-4">
              <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-brand-primary text-white shadow-md shadow-brand-primary/20">
                <SparklesIcon />
              </div>
              <div>
            ...
            <button onClick={() => setIsSidebarOpen(false)} className="flex h-11 w-11 items-center justify-center rounded-2xl text-brand-textMuted hover:bg-brand-bgLight lg:hidden" aria-label="Close admin menu">
              <X size={18} />
            </button>
          </div>
            ...
            {sections.map((section) => {
              ...
              return (
                <button
                  ...
                  className={cn("btn-press flex w-full items-center gap-4 rounded-2xl px-4 py-3 text-left text-sm font-medium transition", activeSection === section.id ? "bg-brand-primary text-white shadow-md shadow-brand-primary/20" : "text-brand-textMuted hover:bg-brand-bgLight hover:text-brand-textPrimary")}
                >
                  <Icon size={17} />
                  {section.label}
                </button>
          ...
          <div className="border-t border-brand-border p-4">
            <Link href="/" className="mb-1 flex items-center gap-4 rounded-2xl px-4 py-3 text-sm font-medium text-brand-textMuted transition hover:bg-brand-bgLight hover:text-brand-primary">
              <Eye size={17} /> View storefront
            </Link>
            <button onClick={handleLogout} className="btn-press flex w-full items-center gap-4 rounded-2xl px-4 py-3 text-sm font-medium text-admin-error transition hover:bg-admin-error-light">
      ...
      <div className="lg:pl-[280px]">
        <header role="banner" className="sticky top-0 z-admin-header border-b border-brand-border bg-cream-50/95 px-4 py-4 md:px-6">
          <div className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-3 md:gap-4">
              <button onClick={() => setIsSidebarOpen(true)} className="flex h-11 w-11 items-center justify-center rounded-2xl border border-brand-border bg-cream-50 text-brand-textPrimary shadow-sm lg:hidden" aria-label="Open admin menu">
                <Menu size={18} />
              </button>
              <div>
            ...
            <div className="hidden flex-1 items-center justify-center md:flex">
              <div className="relative w-full max-w-lg">
                ...
                <input aria-label="Search orders and products" className="w-full rounded-2xl border border-brand-border bg-brand-bgLight py-3 pl-10 pr-4 text-sm outline-none transition focus:border-brand-primary focus:ring-2 focus:ring-brand-primary/10" placeholder="Search products, orders or customers" />
              </div>
            </div>
            <div className="flex items-center gap-2">
              <span className="hidden items-center gap-2 rounded-2xl bg-admin-success-light px-3 py-2 text-xs font-bold text-admin-success md:inline-flex">
                <span className="h-1.5 w-1.5 rounded-2xl bg-admin-success" /> Open
              </span>
              <button className="flex h-11 w-11 items-center justify-center rounded-2xl border border-brand-border bg-cream-50 text-brand-textMuted transition hover:text-brand-primary shadow-sm" aria-label="Notifications">
                <Bell size={16} />
              </button>
              <div className="hidden items-center gap-2 rounded-2xl bg-cream-50 py-2 pl-2 pr-3 shadow-sm md:flex">
                <div className="flex h-8 w-8 items-center justify-center rounded-2xl bg-brand-primary text-xs font-bold text-white">GA</div>
        ...
        <main id="admin-content" aria-label="Admin dashboard" className="p-4 pb-24 md:p-6 md:pb-6 space-y-6">
            ...
            <div className="space-y-6">
              <section className="overflow-hidden rounded-2xl bg-brand-bgDark text-white shadow-lg">
                <div className="grid gap-6 p-5 md:p-6 xl:grid-cols-[1fr_0.78fr] xl:items-center">
                  <div>
                    <span className="font-label inline-flex items-center gap-2 rounded-2xl bg-cream-50/10 px-3 py-1.5 text-[10px] font-bold uppercase tracking-[0.18em] text-white/75"><ShieldCheck size={14} /> Protected workspace</span>
                    ...
                    <div className="mt-5 flex flex-wrap gap-3">
                      <button onClick={() => setActiveSection("products")} className="btn-press rounded-2xl bg-cream-50 px-4 py-3 text-sm font-bold text-brand-primary">Manage products</button>
                      <button onClick={() => setActiveSection("banners")} className="btn-press rounded-2xl border border-white/20 bg-cream-50/10 px-4 py-3 text-sm font-bold text-white">Replace banners</button>
                    </div>
                  </div>
              ...
              <section className="grid gap-6 lg:grid-cols-[1.25fr_0.75fr]">
                <div className="rounded-2xl border border-brand-border bg-cream-50 p-6 shadow-sm">
                  <div className="mb-4 flex items-center justify-between gap-4">
                    ...
                    <button onClick={() => setActiveSection("orders")} className="btn-press rounded-2xl border border-brand-border px-4 py-2 text-sm font-bold text-brand-primary min-h-[44px]">View all</button>
                  </div>
                      ...
                      <tbody>
                        {orderRows.map((order) => (
                          <tr key={order.id} className="border-b border-brand-border/70 last:border-0">
                            ...
                            <td className="px-4 py-4"><button aria-label="Open order actions" className="flex h-11 w-11 items-center justify-center rounded-2xl text-brand-textMuted hover:bg-brand-bgLight"><MoreHorizontal size={16} /></button></td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                ...
                <div className="space-y-6">
                  <div className="card-hover rounded-2xl border border-brand-border bg-cream-50 p-6 shadow-sm">
          ...
          {activeSection === "products" ? (
            <section className="rounded-2xl border border-brand-border bg-cream-50 p-6 shadow-sm">
              <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                ...
                <div className="flex flex-wrap gap-2">
                  <ComingSoonTooltip><button disabled className="btn-press inline-flex items-center gap-2 rounded-2xl border border-brand-border px-4 py-2 text-sm font-medium text-brand-textPrimary disabled:opacity-50 disabled:cursor-not-allowed"><Filter size={15} /> Filter</button></ComingSoonTooltip>
                  <button onClick={exportProductsCsv} className="btn-press inline-flex items-center gap-2 rounded-2xl border border-brand-border px-4 py-2 text-sm font-medium text-brand-textPrimary"><Download size={15} /> Export</button>
                  <ComingSoonTooltip><button disabled className="btn-press inline-flex items-center gap-2 rounded-2xl bg-brand-primary px-4 py-2 text-sm font-medium text-white disabled:opacity-50 disabled:cursor-not-allowed"><Plus size={15} /> Add product</button></ComingSoonTooltip>
                </div>
              </div>
              <div className="relative mt-4 max-w-lg">
                <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 text-brand-textMuted" size={16} />
                <input aria-label="Search products by SKU, brand or name" value={productQuery} onChange={(event) => setProductQuery(event.target.value)} className="w-full rounded-2xl border border-brand-border bg-brand-bgLight py-3 pl-10 pr-4 text-sm outline-none focus:border-brand-primary focus:ring-2 focus:ring-brand-primary/10" placeholder="Search by SKU, brand or product" />
              </div>
...
<td className="px-4 py-4">
                             <div className="flex items-center gap-4">
                               <NextImage src={product.image} alt="" width={40} height={40} className="h-10 w-10 rounded-2xl bg-brand-bgLight object-cover" />
                               <div>
...
<td className="px-4 py-4"><StatusPill className={status === "Active" ? "bg-admin-success-light text-admin-success ring-admin-success/20" : status === "Low" ? "bg-admin-warning-light text-admin-warning ring-admin-warning/20" : "bg-admin-error-light text-admin-error ring-admin-error/20"}>{status}</StatusPill></td>
                           <td className="px-4 py-4">
                             <div className="flex gap-1">
                               <ComingSoonTooltip><button disabled aria-label="View product" className="flex h-11 w-11 items-center justify-center rounded-2xl text-brand-textMuted hover:bg-brand-bgLight disabled:opacity-50 disabled:cursor-not-allowed"><Eye size={15} /></button></ComingSoonTooltip>
                               <ComingSoonTooltip><button disabled aria-label="Edit product" className="flex h-11 w-11 items-center justify-center rounded-2xl text-brand-textMuted hover:bg-brand-bgLight disabled:opacity-50 disabled:cursor-not-allowed"><Pencil size={15} /></button></ComingSoonTooltip>
                               <ComingSoonTooltip><button disabled aria-label="Delete product" className="flex h-11 w-11 items-center justify-center rounded-2xl text-admin-error hover:bg-admin-error-light disabled:opacity-50 disabled:cursor-not-allowed"><Trash2 size={15} /></button></ComingSoonTooltip>
                            </div>
                          </td>
                        </tr>
                      );
          ...
          {activeSection === "orders" ? (
            <section className="rounded-2xl border border-brand-border bg-cream-50 p-6 shadow-sm">
              <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                ...
                <div className="flex flex-wrap gap-2"><ComingSoonTooltip><button disabled className="btn-press rounded-2xl border border-brand-border px-4 py-2 text-sm font-medium disabled:opacity-50 disabled:cursor-not-allowed">Today</button></ComingSoonTooltip><ComingSoonTooltip><button disabled className="btn-press rounded-2xl border border-brand-border px-4 py-2 text-sm font-medium disabled:opacity-50 disabled:cursor-not-allowed">This week</button></ComingSoonTooltip><ComingSoonTooltip><button disabled className="btn-press rounded-2xl bg-brand-primary px-4 py-2 text-sm font-medium text-white disabled:opacity-50 disabled:cursor-not-allowed">Create manual order</button></ComingSoonTooltip></div>
              </div>
              <div className="mt-4 overflow-x-auto -mx-6 px-6">
                <table className="w-full min-w-[900px] text-sm">
...
<td className="px-4 py-4 font-mono text-xs font-semibold">{order.orderNumber}</td><td className="px-4 py-4">{order.date}</td><td className="px-4 py-4">{order.items.length}</td><td className="px-4 py-4">{order.paymentMethod}</td><td className="px-4 py-4 max-w-[200px] truncate">{order.shippingAddress}</td><td className="px-4 py-4 font-semibold">{formatNPR(order.total)}</td>
                       <td className="px-4 py-4"><select aria-label="Order status" value={order.status} onChange={(event) => setOrderStatusById((current) => ({ ...current, [order.id]: event.target.value as Order["status"] }))} className="rounded-2xl border border-brand-border bg-cream-50 px-3 py-2 text-xs font-medium outline-none focus:border-brand-primary focus:ring-2 focus:ring-brand-primary/10"><option>Pending</option><option>Confirmed</option><option>Processing</option><option>Shipped</option><option>Delivered</option><option>Cancelled</option></select></td>
                    </tr>
                  ))}</tbody>
                </table>
              </div>
            ...
            <section className="grid gap-6 lg:grid-cols-[1fr_0.75fr]">
              <div className="rounded-2xl border border-brand-border bg-cream-50 p-6 shadow-sm">
                ...
                <div className="mt-5 space-y-2">
                  {LOW_STOCK_SNAPSHOT.map((item) => <div key={item.productId} className="flex flex-col gap-2 rounded-2xl border border-brand-border p-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-semibold">{item.name}</p><p className="text-xs text-brand-textMuted">{item.sku} · Reorder {item.reorderPoint} · Target {item.restockTarget}</p></div><div className="flex items-center gap-2"><StatusPill className={riskStyles[item.risk]}>{item.risk}</StatusPill><ComingSoonTooltip><button disabled className="btn-press rounded-2xl bg-brand-primary px-4 py-2 text-xs font-medium text-white disabled:opacity-50 disabled:cursor-not-allowed">Restock</button></ComingSoonTooltip></div></div>)}
                </div>
              </div>
              <div className="rounded-2xl border border-brand-border bg-cream-50 p-6 shadow-sm"><h3 className="font-display text-xl font-semibold">Inventory rules</h3><div className="mt-4 space-y-4 text-sm text-brand-textMuted"><p className="flex gap-3"><CheckCircle2 className="mt-0.5 shrink-0 text-admin-success" size={16} /> Best sellers should trigger reorder at 30 units.</p><p className="flex gap-3"><CheckCircle2 className="mt-0.5 shrink-0 text-admin-success" size={16} /> Show customer-safe availability labels only.</p><p className="flex gap-3"><AlertTriangle className="mt-0.5 shrink-0 text-admin-warning" size={16} /> Connect inventory ledger before accepting real orders.</p></div></div>
            </section>
          ) : null}
          ...
          {activeSection === "banners" ? (
            <section className="grid gap-6 lg:grid-cols-[1fr_0.76fr]">
              <div className="space-y-6">
                <div className="rounded-2xl border border-brand-border bg-cream-50 p-6 shadow-sm">
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="font-display text-2xl font-semibold">Banner manager</h2><p className="mt-0.5 text-sm text-brand-textMuted">Replace homepage and campaign banners with adaptive desktop and mobile assets.</p></div><button onClick={() => setBannerMessage("Banner settings saved in this browser. Connect the admin API to publish across devices.")} className="btn-press inline-flex items-center gap-2 rounded-2xl bg-brand-primary px-4 py-3 text-sm font-medium text-white"><Save size={15} /> Save banner</button></div>
                  <div className="mt-4 grid gap-2 sm:grid-cols-2">{banners.map((banner) => <button key={banner.id} onClick={() => setSelectedBannerId(banner.id)} className={cn("rounded-2xl border p-3 text-left transition", selectedBanner.id === banner.id ? "border-brand-primary bg-brand-primary-light" : "border-brand-border bg-cream-50 hover:bg-brand-bgLight")}><p className="font-semibold">{banner.title}</p><p className="mt-0.5 text-[11px] text-brand-textMuted">{banner.status} · Updated {banner.updatedAt}</p></button>)}</div>
                </div>
                <BannerPreview banner={selectedBanner} />
                <div className="rounded-2xl border border-brand-border bg-cream-50 p-5 shadow-sm">
                  <h3 className="font-display text-xl font-semibold">Edit selected banner</h3>
                  <div className="mt-4 grid gap-3 sm:grid-cols-2">
<label htmlFor="banner-title" className="space-y-2 text-sm font-medium">Title<input id="banner-title" value={selectedBanner.title} onChange={(event) => updateBannerField("title", event.target.value)} className="w-full rounded-2xl border border-brand-border px-4 py-3 text-sm outline-none focus:border-brand-primary focus:ring-2 focus:ring-brand-primary/10" /></label>
                     <label htmlFor="banner-cta" className="space-y-2 text-sm font-medium">CTA text<input id="banner-cta" value={selectedBanner.cta} onChange={(event) => updateBannerField("cta", event.target.value)} className="w-full rounded-2xl border border-brand-border px-4 py-3 text-sm outline-none focus:border-brand-primary focus:ring-2 focus:ring-brand-primary/10" /></label>
                     <label htmlFor="banner-subtitle" className="space-y-2 text-sm font-medium sm:col-span-2">Subtitle<textarea id="banner-subtitle" value={selectedBanner.subtitle} onChange={(event) => updateBannerField("subtitle", event.target.value)} className="mt-1 min-h-[80px] w-full rounded-2xl border border-brand-border px-4 py-3 text-sm outline-none focus:border-brand-primary focus:ring-2 focus:ring-brand-primary/10" /></label>
                     <label htmlFor="banner-link" className="space-y-2 text-sm font-medium">Link<input id="banner-link" value={selectedBanner.href} onChange={(event) => updateBannerField("href", event.target.value)} className="w-full rounded-2xl border border-brand-border px-4 py-3 text-sm outline-none focus:border-brand-primary focus:ring-2 focus:ring-brand-primary/10" /></label>
                     <label htmlFor="banner-status" className="space-y-2 text-sm font-medium">Status<select id="banner-status" value={selectedBanner.status} onChange={(event) => updateBannerField("status", event.target.value as ManagedBanner["status"])} className="w-full rounded-2xl border border-brand-border px-4 py-3 text-sm outline-none focus:border-brand-primary focus:ring-2 focus:ring-brand-primary/10"><option>Published</option><option>Scheduled</option><option>Paused</option></select></label>
                  </div>
                </div>
              </div>
              <aside className="space-y-6">
                <div className="rounded-2xl border border-brand-border bg-cream-50 p-6 shadow-sm"><Upload className="text-brand-primary" size={20} /><h3 className="mt-2 font-display text-xl font-semibold">Upload assets</h3><p className="mt-2 text-sm leading-6 text-brand-textMuted">Desktop: 16:7 ratio (1920 x 840). Mobile: 4:5 ratio (1080 x 1350). PNG, JPG, WebP, SVG under 3 MB.</p><div className="mt-4 space-y-3"><label className="block rounded-2xl border border-dashed border-brand-primary/40 bg-brand-primary-light p-4 text-sm font-medium text-brand-primary cursor-pointer"><input type="file" accept="image/png,image/jpeg,image/webp,image/svg+xml" onChange={(event) => handleBannerUpload(event, "desktop")} className="hidden" />Upload desktop banner</label><label className="block rounded-2xl border border-dashed border-brand-primary/40 bg-brand-primary-light p-4 text-sm font-medium text-brand-primary cursor-pointer"><input type="file" accept="image/png,image/jpeg,image/webp,image/svg+xml" onChange={(event) => handleBannerUpload(event, "mobile")} className="hidden" />Upload mobile banner</label></div><div aria-live="polite">{uploadError ? <p className="mt-4 rounded-2xl bg-admin-error-light p-3 text-sm font-medium text-admin-error">{uploadError}</p> : null}{bannerMessage ? <p className="mt-4 rounded-2xl bg-admin-success-light p-3 text-sm font-medium text-admin-success">{bannerMessage}</p> : null}</div></div>
...
function SettingItem({ icon: Icon, label, value }: { icon: ComponentType<{ size?: number | string; className?: string }>; label: string; value: string }) {
  return <div className="rounded-2xl border border-brand-border bg-brand-bgLight p-4"><div className="flex items-start gap-3"><Icon className="mt-0.5 text-brand-primary" size={16} /><div><p className="font-label text-xs font-bold uppercase tracking-[0.14em] text-brand-textMuted">{label}</p><p className="mt-1 text-sm font-medium text-brand-textPrimary">{value}</p></div></div></div>;
}
