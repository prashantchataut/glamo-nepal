...
export function DashboardView() {
    ...
    return (
      ...
      <section className="rounded-[1.5rem] border border-brand-border bg-white p-6 shadow-sm">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <p className="text-sm font-semibold text-brand-textPrimary">Today</p>
          ...
          <Link href="/admin/support" className="btn-press inline-flex w-fit rounded-full bg-brand-primary px-4 py-3 text-sm font-bold text-neutral-50">Open support inbox</Link>
        </div>
        <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-5">
          {todayActions.map((action) => {
            const Icon = action.icon;
            return (
              <Link key={action.label} href={action.href} className="rounded-[1.5rem] border border-brand-border p-4 transition hover:bg-brand-bgLight">
  ...
  if (isError && !stats) {
    return (
      <section className="rounded-[1.5rem] border border-red-200 bg-red-50 p-6 text-center">
        ...
        <button
          type="button"
          onClick={() => refetch()}
          className="mt-4 inline-flex items-center justify-center rounded-full bg-brand-primary px-6 py-2.5 text-sm font-semibold text-neutral-50 transition hover:bg-brand-bgDark"
        >
          Retry
        </button>
      </section>
  ...
  return (
    <div className="space-y-6">
      <section className="overflow-hidden rounded-[1.5rem] bg-brand-bgDark text-neutral-50 shadow-lg">
        <div className="grid gap-6 p-5 md:p-6 xl:grid-cols-[1fr_0.78fr] xl:items-center">
          <div>
            <span className="font-label inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-1.5 text-sm font-semibold text-neutral-50/75">
            ...
            <p className="mt-3 max-w-3xl text-sm leading-6 text-neutral-50/70">
            ...
            <div className="mt-5 flex flex-wrap gap-3">
              ...
              <Link
                href="/admin/content"
                className="btn-press rounded-full border border-white/20 bg-white/10 px-4 py-3 text-sm font-bold text-neutral-50"
              >
                Update content
              </Link>
            </div>
            ...
            <div className="mt-5 flex h-32 items-end gap-1.5">
              {revenueDays.length > 0 ? revenueDays.map(([date, d]) => (
                <div key={date} className="flex flex-1 flex-col items-center gap-2">
                  ...
                  <span className="text-xs text-brand-textMuted">{date.slice(5)}</span>
                </div>
              )) : <p className="self-center text-sm text-brand-textMuted">No paid revenue yet.</p>}
            </div>
          </div>
          <div className="card-hover rounded-[1.5rem] border border-brand-border bg-white p-6 shadow-sm">
                ...
                lowStockProducts.slice(0, 4).map((item) => (
                  <div key={item.id} className="flex items-center justify-between gap-3 rounded-xl bg-brand-bgLight p-3 text-sm">
                    <div>
                      ...
                      <p className="text-xs text-brand-textMuted">Reorder at {item.low_stock_threshold}</p>
                    </div>
