  ...
  return (
    <div>
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        ...
        <Link href="/shop" className="inline-flex items-center justify-center gap-2 rounded-2xl bg-brand-primary px-6 py-3 text-sm font-semibold text-white transition hover:bg-brand-primary-hover"><ShoppingBag size={16} /> Continue shopping</Link>
      </div>
      ...
      <div className="mt-8 grid gap-4">
        {orders.map((order) => {
          ...
          const card = (
            <article className="group rounded-2xl border border-brand-border bg-cream-50 p-4 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md md:p-5">
                ...
                <div className="flex min-w-0 items-center gap-4">
                  ...
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <span className={cn("font-label rounded-2xl px-3 py-1 text-[10px] font-bold uppercase tracking-[0.14em] ring-1", statusStyles[order.status] || "bg-brand-bgLight text-brand-textMuted ring-brand-border")}>{order.status}</span>
                      {order.source === "session" ? <span className="font-label rounded-2xl bg-brand-primary-light px-3 py-1 text-[10px] font-bold uppercase tracking-[0.14em] text-brand-primary">New</span> : null}
                    </div>
