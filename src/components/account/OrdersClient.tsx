  ...
  return (
    <div>
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.22em] text-brand-primary">Order history</p>
          <h1 className="mt-2 font-serif text-4xl font-semibold leading-tight text-brand-textPrimary md:text-5xl">Your GLAMO orders</h1>
        ...
        <Link href="/shop" className="inline-flex items-center justify-center gap-2 rounded-full bg-brand-primary px-6 py-3 text-sm font-semibold text-white transition hover:bg-brand-primary-hover"><ShoppingBag size={16} /> Continue shopping</Link>
      </div>
      ...
      <div className="mt-8 grid gap-4">
        {orders.map((order) => {
          ...
          const card = (
            <article className="group rounded-[2rem] border border-brand-border bg-white p-4 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md md:p-5">
              <div className="flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
                <div className="flex min-w-0 items-center gap-4">
                  <div className="relative h-20 w-20 shrink-0 overflow-hidden rounded-2xl bg-brand-bgLight">
                  ...
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      <h2 className="font-serif text-2xl font-semibold text-brand-textPrimary">{order.orderNumber}</h2>
                      <span className={cn("rounded-full px-3 py-1 text-[10px] font-bold uppercase tracking-[0.14em] ring-1", statusStyles[order.status] || "bg-brand-bgLight text-brand-textMuted ring-brand-border")}>{order.status}</span>
                      {order.source === "session" ? <span className="rounded-full bg-brand-primary-light px-3 py-1 text-[10px] font-bold uppercase tracking-[0.14em] text-brand-primary">New</span> : null}
                    </div>
