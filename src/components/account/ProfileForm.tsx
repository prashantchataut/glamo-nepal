...
export function ProfileForm() {
  ...
  const [phone, setPhone] = useState(SAMPLE_USER.phone);
  ...
  return (
    <form onSubmit={submit} className="rounded-[2rem] border border-border/70 bg-white p-6 shadow-sm md:p-8">
      ...
      <div className="mt-6 grid gap-5 md:grid-cols-2">
        ...
        <label className="text-sm font-semibold text-brand-textPrimary">Email<input value={SAMPLE_USER.email} readOnly className="mt-2 w-full cursor-not-allowed rounded-2xl border border-border bg-brand-bgLight px-4 py-3 text-brand-textMuted outline-none" /></label>
