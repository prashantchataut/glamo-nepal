...
export function CodAvailabilityChecker({ district, province }: { district: string; province?: string }) {
  ...
  return (
    <div className={cn("rounded-2xl border p-4", rule.codAvailable ? "border-emerald-200 bg-emerald-50 text-emerald-900" : "border-amber-200 bg-amber-50 text-amber-900")}>
      <div className="flex items-start gap-3">
        {rule.codAvailable ? <CheckCircle2 className="mt-0.5" size={20} /> : <XCircle className="mt-0.5" size={20} />}
        <div>
